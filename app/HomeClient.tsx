...
const MovieRow = dynamic(() => import("@/components/MovieRow"), {
  loading: () => (
    <div className="h-64 animate-pulse bg-zinc-900/50 rounded-xl my-8"></div>
  ),
});
const HistoryTontonan = dynamic(() => import("@/Fragments/HistoryWatch"), {
  ssr: false, // History only exists on client side (localStorage)
});
...
export default function HomeClient({ initialPopularMovies }: HomeClientProps) {
  ...
  const { data: popularMovies, isLoading: isPopLoading } = useQuery({
    ...
    initialData: initialPopularMovies, // Use data fetched from Server Component
    ...
    fetch("https://ipapi.co/json/")
      ...
      .then((data) => {
        if (data?.country_code) {
          ...
          localStorage.setItem("userRegion", data.country_code);
        }
      })
      .catch(() => {
  ...
  const isMainLoading = isRegLoading; // Popular movies are initially loaded
